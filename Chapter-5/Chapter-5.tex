...
On the backdrop of privacy concerns and a rising number of privacy breach incidents, new regulations and standards such as the General Data Protection Regulation (GDPR)\footnote{GDPR, \url{https://eur-lex.europa.eu/eli/reg/2016/679/oj}} are being introduced. 
...
\end{enuminline}
% More often than not these concerns are not addressed when sharing
% decisions are made today as users have little sense of privacy (and other values) when they share information on the web. \mps{Blames users. You mean today's approaches do not provide awareness of privacy consequence?} \nsa{Spiekermann suggests giving control to users but we argue users are not capable of making good decisions. }\mps{Sounds intense to say users are incapable but maybe an argument that the decision making burden is excessive and so SIPAs can help users and sometimes automate decision}
...
The concept of \fsl{value} has two different connotations: one is about economic worth of something and the other, much broader, meaning refers to what people consider as important in their lives \citep{Friedman-2008-value-sensitive-design}. 
...
and \citet{rokeach1973nature}. The values in Schwartz's \shortcite{schwartz2012overview} work are broad motivational goals such as stimulation, achievement, security, and benevolence. \citet{rokeach1973nature} proposes two types of values---\fsl{terminal} and \fsl{instrumental}. Terminal values  such as security, freedom, happiness, and recognition, refer to defined-end states of existence. Instrumental values refer to modes of behavior or means to achieve the terminal values. 
%
...
% We consider privacy as a desired state where people \mps{all people together? not privacy of an individual but of everyone together? That's a mistake the legal types talk about because they are interested in policy and law; our approach is much more fine grained} have control over the concealing or revelation of their personal information. 
...
% Privacy is an ancient concept, but new information technologies are consistently reforming its implications \mps{these words don't sound like your normal language}, requiring the implementation of new regulations. 
...
We adopt Singh's \shortcite{Singh-2013-Norms} representation of social norms, in which a norm is directed from a subject to an object and is constructed as a conditional relationship involving an antecedent (which brings an instance of the norm in force) and a consequent (which brings the norm instance to completion). A norm generates a new instance each time it applies. 
...
A commitment norm means its subject is committed to its object to bring about a consequent if an antecedent holds, and a prohibition norm means its subject is forbidden by its object to bring about a consequent if an antecedent holds. For instance, \fsl{Frank} (subject), a high school student is \fsl{committed} (norm) to \fsl{Grace} (object), his mother, that he \fsl{will keep Grace updated about his location} (consequent) when he is \fsl{away from home} (antecedent).
...
% \frameworkAinur involves group \mps{you said you removed it?} decision-making. 
...
% Fearing a negative sanction from Harold to Frank, 
...
\end{tikzpicture}
\caption[A conceptual model of an \frameworkAinur SIPA]{A conceptual model of an \frameworkAinur SIPA.}
\label{fig:valar}
\end{figure}
...
% \mps{Sudden para. Not clear how it relate to context} \nsa{Revised}
\end{itemize}

\subsubsection{Social model} includes norms governing a SIPA's interactions in a society and the associated sanctions. 
\begin{itemize}
...
% The satisfaction or violation of a norm could promote or demote certain
% values. There could be conflicts---values promoted by norms conflict
% with values promoted by agents goals. Agents can choose to satisfy or
% violate norms by identifying stakeholders' contextual preferences of
% values that these norms promote or demote. 

% \nsa{\citet{rawls1985justice}
% \citet{Leben2017Rawls}
% \citet{leben2018ethics}}
...
% 
For instance, in a society of \locationapp SIPAs, when a stakeholder moves to a new place or meets new people, his or her SIPA may share the context to bring about the stakeholder's goal $g$ of \fsl{staying connected}. The context includes the place where a stakeholder is and the people who he or she is with. 
...
% When sharing the context, the SIPA may also tag companions. 
...
% Each place $l$ can be defined by attributes such as safety or sensitivity. 
...
% $V$ is a set of terminal values defined by \citet{rokeach1973nature}. \nsa{Instead of Rokeach values, it could be all values that are influenced in the current context.} 
...
%Each agent $a_i$ has a contextual preference $q_i$ over values,
%represented by a set of tuples $\{(v_{i_x}^k, v_{i_y}^k, c_{k}) \mid v_{i_x}^k, v_{i_y}^k \in V, c_k
%\in C\}$ such that $a_i$ prefers $v_{i_x}^k$ over $v_{i_y}^k$ in $c_k$.

During each context $c$, each agent $a$ has a contextual preference $q$ over values,
...
%Each SIPA is equipped with sensors via which, first, it gains an
%understanding of the environmental context and current state of the
%world. Second, it determines the future state of the world for each
%action it could perform. Third, given an understanding of the value
%preferences of its stakeholders, the SIPA determines the social
%experience its stakeholders will derive if it performs the selected
%action. Fourth, based on the applicable norms in a given context and its
%stakeholders goals, a SIPA identifies the action to perform.
...
\item[Actions.] To help bring about their stakeholders' goal of being connected, as stakeholders move, their SIPAs either \fsl{share their context publicly}, \fsl{share only among common friends of all companions}, or \fsl{share only with companions}. Context sharing actions are based on norms and stakeholders' value preferences.

\item[Norm.] The society is governed by a privacy norm---\fsl{preserve privacy of all stakeholders}, i.e., stakeholders are committed to act in a privacy-preserving manner when sharing context while accompanying others. This commitment is directed from the primary stakeholders to the secondary stakeholders.  
...
A \fbf{privacy fundamentalist} stakeholder is most protective about his or her privacy. 

A \fbf{privacy unconcerned} stakeholder is inclined to share information about himself or herself. He or she perceives more benefit from sharing information than holding it. 

A \fbf{privacy pragmatist} stakeholder exhibits a pragmatic behavior, and weighs pro and cons of sharing information based on a given context. 
...
First, the participants completed a privacy attitude survey \citep{schnorf2014comparison} \nsa{refer appendix} in which they answered questions on their level of comfort in sharing personal information on the Internet on a Likert scale of 1 (very comfortable) to 5 (very uncomfortable), and the extent sharing personal information causes (or could cause) them negative experience, again on a Likert scale of 1 (not at all) to 5 (to a very great extent). Figure~\ref{fig:participants-privacy-distribution} shows the distribution of privacy attitudes of the study participants. 
...
%  	width=8cm,
%  	xtick={1,3,5},
%     xticklabels={Unconcerned,Pragmatist,Fundamentalist},
%  	xlabel={Privacy Attitude},
%  	xmin=1, xmax=5,
% %     xticklabels = {Unconcerned,,,, Fundamentalist},
...
%   \end{tikzpicture}
%   \caption{Privacy attitudes of the participants. \nsa{Replace with a bean plot that shows the distribution.}}
...
\bottomrule
% \multicolumn{7}{p{8cm}}{Ple., Pri., Rec., and Sec. are pleasure, privacy, recognition, and security respectively.}
\end{tabular}
\end{table}
...
% \mps{explain path length and clustering}
% \nsa{}
...
% \nsa{Replace with small world network}
...
%     legend pos=south east,
    ...
    {./Chapter-5/data/weightedresults.csv};
    \addplot +[mark size=1, green!30!black, dashed] table [x=tick, y=conservative, col sep=comma]
    {./Chapter-5/data/weightedresults.csv};
    \addplot +[mark size=1, densely dashdotted] table [x=tick, y=majority, col sep=comma]
    {./Chapter-5/data/weightedresults.csv};
    \legend{S\fsub{\frameworkAinur},S\fsub{primary},S\fsub{conservative},S\fsub{majority}}
    \end{axis}
    \end{tikzpicture}
...
%   mark=none,domain=0:5,samples=50,smooth}, % All plots: from -2:2, 50 samples, smooth, no marks
% %   axis x line*=bottom, % no box around the plot, only x and y axis
% %   axis y line*=left, % the * suppresses the arrow tips
%   xtick={1,3,5},
%   xticklabels={Unconcerneds, Pragmatists, Fundamentalists},
%   ylabel={\# of Stakeholders},
%   yticklabels={},
%   xmin=1, xmax=5,
%   ymin=0, ymax=1,
%   height=6cm, width=6cm,
% %   enlargelimits=upper,
% %   legend style={at={(1.85,0.2)}, anchor=east},
%   legend style={at={(0.5,-0.2)},anchor=north,},
%   legend columns=3, 
%   ] % extend the axes a bit to the right and top
%   \addplot {gauss(3.0,0.5)};
%   \addplot {gauss(2.0,0.5)};
%   \addplot {gauss(4.0,0.5)};
%   \legend{Fundamentalists, Pragmatists, Unconcerneds}
% \end{axis}
...
\begin{axis}[
    ...
    % ylabel={Society},
    xtick = {1,5},
    xticklabels={Highly unconcerned, Highly concerned},
    yticklabel style={rotate=45},
    ytick={1,2,3},
    yticklabels={Fundamentalists, Pragmatists, Unconcerneds},
...
\end{axis}
\end{tikzpicture}
\caption[Privacy attitude distributions for societies]{Privacy attitude distributions for societies of fundamentalists, pragmatists, and unconcerneds.}
\label{fig:privacy-distribution-experiment}
\end{figure}

Table~\ref{tab:result-privacy} summarizes the results for the experiments with privacy fundamentalist, pragmatist, and unconcerned societies. Figure~\ref{fig:experience-plot+privacy} shows the social experience plots for these experiments.
...
\begin{figure}[!htb]
    ...
    title={Privacy Fundamentalist Society},
    height=5cm,
    width=7cm,
    xlabel={Time in 100 steps},
...
%     
    \begin{tikzpicture}
    \begin{axis}[
    title={Privacy Pragmatist Society},
    height=5cm,
    width=7cm,
    xlabel={Time in 100 steps},
%     ytick={},
...
%
    \begin{tikzpicture}
    \begin{axis}[
    title={Privacy Unconcerned Society},
    height=5cm,
    width=7cm,
    xlabel={Time in 100 steps},
%     ytick={},
...
The focus in \citet{IJCAI-18:Poros}'s work is to maximize the social experience of its users. Maximizing social experience may not translate to fairness as we observed in experiments with privacy fundamentalist society where S\fsub{majority} yields maximum social experience but least fairness. 
% \mps{meaning?} \nsa{like resource starvation in OS; maximizing total may result in some stakeholders always receiving lowest utility? For example, S-majority in Fundamentalist society gets highest mean social experience but the fairness is low.} 
% \frameworkAinur's focus is on fairness in choosing ethically appropriate actions.
\frameworkAinur's focus is to balance the needs of both primary and secondary users. 

\citet{Ajmeri-AAMAS17-Arnor} provide an approach for engineering agents who promote privacy. Their approach lacks constructs to model stakeholders' values. Our approach, \frameworkAinur, including understanding of value preferences to aid decision-making, and its evaluation are novel.
% 
...
% , which are relevant to our work.\mps{how?} 
% 
...
% A second direction is to crowdsource data about values and decision-making about sharing policies on a much larger scale. 
...
% \mps{Kayal's bib entry is incomplete}
...
% \bibliography{Munindar,Nirav,Pradeep}
